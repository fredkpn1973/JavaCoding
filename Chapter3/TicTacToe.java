...
public class TicTacToe {
    ...
    public void putMark(int i, int j, boolean played) throws IllegalArgumentException, IllegalStateException {
        if ((i < 0) || (i > 2) || (j < 0) || (j > 2))
            throw new IllegalArgumentException("Invalid board position");
        if (board[i][j] != EMPTY)
            throw new IllegalArgumentException("Board position occupied");
        if (played)
            throw new IllegalStateException("Game Already played");
    ...
    public static void main(String[] args) {
        ...
        /* X moves: */ /* O moves: */
        boolean gamplayed = false;
        int moves = 0;
        while (!gamplayed) {
            game.putMark(0, 0, gamplayed);
            game.putMark(2, 2, gamplayed);
            game.putMark(0, 1, gamplayed);

            game.putMark(0, 2, gamplayed);
            game.putMark(2, 1, gamplayed);
            game.putMark(1, 2, gamplayed);
            game.putMark(1, 0, gamplayed);
            gamplayed = true;
        }

        System.out.println(game);
        int winningPlayer = game.winner();
        game.putMark(1, 1, gamplayed);
